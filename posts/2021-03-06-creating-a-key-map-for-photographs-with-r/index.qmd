...
image_table <-
  ...
  tab_style(
    ...
    locations = cells_body(columns = c(title))
  ) %>%
...
The next step is to make the map base layer using the `layer_mapbox` function from the maplayer package. For this to work, you need to a [Mapbox API](https://docs.mapbox.com/api/overview/) key to pass to the function or to save it to your local environment using `mapboxapi::mb_access_token()`.[^1]
...
make_flickr_key_table <- function(x, table_title = NULL) {
  x %>%
    ...
    tab_header(
      ...
      subtitle = html(
        ggplot_image(
          make_key_map(x, dist = 50, asp = "4:3"),
          height = 480
        )
      )
    ) %>%
...
Getting the photos from Flickr requires both the [FlickrAPI package](https://github.com/elipousson/FlickrAPI/)[^3] and a [Flickr API key](https://www.flickr.com/services/apps/create/apply). You can save the key to the local environment with `Sys.setenv(FLICKR_ACCESS_TOKEN = "x")` (again substituting your own API key for x). I'm using the `getPhotoSearch` function to pull images from the [Baltimore Heritage Flickr account](https://www.flickr.com/photos/baltimoreheritage/) tagged with the Eager Park, Ellwood Park, or McElderry Park neighborhood names.

```{r flickr}
# remotes::install_github("elipousson/FlickrAPI")
library(FlickrAPI)

flickr_photos <- get_photo_search(
...
[^1]: If you want to use a base map that isn't available from Mapbox, you may want to take a look at the [ggmap](https://github.com/dkahle/ggmap) package which has a few more options. You can also make a custom basemaps using the [Mapbox studio](https://studio.mapbox.com/).

[^2]: I ordered these photos by longitude so, in Baltimore, the numbers appear on the map in order from left to right. If I sorted in descending order by latitude instead, the numbers would have appeared on the map from top to bottom.

[^3]: There is a small bug with the getPhotoSearch function in the main repository for this package so I recommend installing my version if you want to run a version of the code below. I'll update this post once I make a pull request and get the bug fixed in the main repository.
